    ...
    public static void PrepareAndShowDialogue(string csvName)
{
    ...
    if (iconManager == null)
    {
        Debug.LogError("[DialogueHelper] Icon_AdaptationManager 를 찾을 수 없습니다.");
        return;
    }
    ...
    if (dm.dialoguePanel == null)
    {
        Debug.LogError("[DialogueHelper] dialoguePanel을 찾을 수 없습니다.");
        return;
    }
    ...
    {
        Debug.LogError("[DialogueHelper] UI 컴포넌트 바인딩 실패: nameText, dialogueText, nextButton 중 하나 이상이 null입니다.");
        return;
    }
    ...
    try
    {
        ...
        // dialoguePanel 강제 활성화
        dm.dialoguePanel.SetActive(true);

        // MouseLook 상태 확인 및 토글
